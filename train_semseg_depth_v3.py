...
def __log_validation_results_wrapper(model, optimizer, data_loader_val, scheduler, rank, train_res_file, device, writer):    
    def __log_validation_results(trainer_engine):
        ...
        weights_path = "{}{}_loss_{}.pth".format(
            constants.MODELS_LOC, "Semseg_Depth_v3", batch_loss)
...
def train(gpu, args):
    ...
    res_filename = "results_{}".format("Semseg_Depth_v3")
    ...
    with open(train_res_file, "w+") as training_results:
        training_results.write(
            "----- TRAINING RESULTS - Vkitti {} ----".format("Semseg_Depth_v3")+"\n")
    # Set device
    ...
    model = models.get_model_by_name("Semseg_Depth_v3").cuda(args.gpu)


        # model.load_state_dict(torch.load(config_kitti.CHECKPOINT_SEMSEG_DEPTH_v3))
    ...
    if config_kitti.CHECKPOINT_SEMSEG_DEPTH_v3 is not None:
        dist.barrier()
        sys.stdout = open(train_res_file, 'a+')
        print("Loading checkpoint from {} to {}".format(0, rank), config_kitti.CHECKPOINT_SEMSEG_DEPTH_v3)
        # map location
        map_location = {'cuda:%d' % 0: 'cuda:%d' % rank}

        checkpoint = torch.load(config_kitti.CHECKPOINT_SEMSEG_DEPTH_v3, map_location=map_location)
    ...
    if rank ==0:
        writer = SummaryWriter(log_dir="runs/Semseg_Depth_v3")
    else:
        writer=None
    # ---------------TRAIN--------------------------------------
