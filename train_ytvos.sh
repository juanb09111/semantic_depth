...
python train.py --model_name=$MODEL_NAME --batch_size=$BATCH_SIZE --checkpoint=$CHECKPOINT --dataset=$DATASET --ann_file=$ANN_FILE --data=$DATA --n_samples=$N_SAMPLES --epochs=$EPOCHS --nodes=1 --ngpus=1 --ip_adress $ip1 $SLURM_TASK_ARRAY_ID