...
def __log_validation_results_wrapper(model, optimizer, data_loader_val, scheduler, rank, train_res_file, device, writer):    
    def __log_validation_results(trainer_engine):
        ...
        weights_path = "{}{}_loss_{}.pth".format(
            constants.MODELS_LOC, "Semseg_Depth_v2_loss_sum", batch_loss)
...
def train(gpu, args):
    ...
    res_filename = "results_{}".format("Semseg_Depth_v2_loss_sum")
    ...
    with open(train_res_file, "w+") as training_results:
        training_results.write(
            "----- TRAINING RESULTS - Vkitti {} ----".format("Semseg_Depth_v2_loss_sum")+"\n")
    # Set device
    ...
    if rank ==0:
        writer = SummaryWriter(log_dir="runs/Semseg_Depth_v2_loss_sum")
    else:
        writer=None
    # ---------------TRAIN--------------------------------------
